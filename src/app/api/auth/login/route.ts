...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!result.success || !result.user) {
      // Check if user is known locally or pending
      const errorMsg = result.error || 'Autentikasi gagal.';
      return NextResponse.json(
    ...
    const isSuper = user.role === 'superadmin' || user.email.toLowerCase() === 'lramdanie02@gmail.com';
    const isPending = user.approval_status && (
      user.approval_status.toLowerCase() === 'pending_approval' ||
    ...
    if (!isSuper && isPending) {
      return NextResponse.json({
        ok: true,
        isPending: true,
        message: 'Akun Anda sedang menunggu persetujuan Super Admin.',
        user: {
          id: user.id,
          email: user.email,
    ...
    response.cookies.set('tatadana_user_status', user.approval_status || 'pending_approval', cookieOptions);
